...
ex.add_config({'run_id': 'train_ctmc_full_gt_w_reid_last',
               'add_date': True,
...
def cfg(cross_val_split, eval_params, dataset_params, graph_model_params, data_splits):
    ...
    if cross_val_split is not None:
        assert cross_val_split in (1, 2, 3, 4), f"{cross_val_split} is not a valid cross validation split"
        # data_splits['train'] =['mot15_train_gt', f'mot17_split_{cross_val_split}_train_gt']
        # data_splits['val'] = [f'split_{cross_val_split}_val']
        data_splits['train'] =[ f'ctmcv1_split_{cross_val_split}_train_gt']
        data_splits['val'] = [f'split_{cross_val_split}_ctmc_val']
